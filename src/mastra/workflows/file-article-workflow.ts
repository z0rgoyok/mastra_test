...
const inputSchema = z.object({
  filePath: z.string().describe('The path to the text file to use as the source.'),
  styleFilePath: z
    .string()
    .optional()
...
const articleCreationStep = createStep({
  ...
  execute: async ({ inputData, runtimeContext }) => {
    const { filePath, styleFilePath, outputFilePath } = inputData;
    ...
    let docContent = '';
    try {
      docContent = await fs.readFile(path.resolve(filePath), 'utf-8');
    } catch (error) {
      console.error(`Error reading source file at ${filePath}:`, error);
      throw new Error('Failed to read the source file.');
    }
    
    tokenUsage.reader = 0; // Assuming the reader tool doesn't consume tokens
    ...
    const totalTokens = tokenUsage.reader + tokenUsage.writer + tokenUsage.editor + tokenUsage.humanizer;
    
    // Display token summary
    console.log('--- Token Usage Summary ---');
    console.log(`Reader: ${tokenUsage.reader} tokens`);
    ...
    const finalOutputPath = outputFilePath || `articles/file_article_${Date.now()}.md`;
...
export const fileArticleWorkflow = createWorkflow({
  id: 'fileArticleWritingWorkflow',
  inputSchema,
  outputSchema,
